...
struct MoviesWantsView: View {
    @FetchRequest(entity: Movie.entity(), sortDescriptors: [], predicate: NSPredicate(format: "seen == %@", NSNumber(value: false))) var movies: FetchedResults<Movie>
    let genres = ["Sci-Fi","Fantasy", "Comedy", "Horror", "Thriller", "Criminal", "Kids", "Romance", "Drama"].sorted()
    
    var body: some View {
        GeometryReader { geometry in
            VStack {
                Text("Wants to see \(self.movies.count) movies")
                    .font(.system(size: 28))
                    .bold()
                    .padding()
